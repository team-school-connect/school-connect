  ...
  mutation: {
    createVolunteerPosition: combineResolvers(
      ...
      async (parent, args, context) => {
        ...
        const volunteerPosition = await VolunteerPosition.findOne({
          organizationName,
          positionName,
        ...
        if (volunteerPosition)
          throw new ConflictError(
            `Volunteer position "${positionName}" at ${location} for organization ${organizationName} already exists`
          );
  ...
  query: {
    ...
    getSingleVolunteerPosition: combineResolvers(
      ...
      async (parent, args, context) => {
        ...
        const singleVolunteerPosition = await VolunteerPosition.findOne({ _id }).exec();

        if (singleVolunteerPosition === null)
          throw new NotFoundError("Volunteer position not found.");

        return singleVolunteerPosition;
      }
    ),
  },
};
